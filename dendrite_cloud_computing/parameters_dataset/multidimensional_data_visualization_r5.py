...
VISUAL_DEFAULTS = {
    ...
    'radial_grid': False,  # Changed default to False for cleaner chord diagrams
...
# ============================================================
# ENHANCED CHORD DIAGRAM WITH FULL PARAMETERIZATION - GRID FIX APPLIED
# ============================================================
...
def create_enhanced_chord_diagram(
    ...
    radial_spacing=0.8,
    radial_label_offset=0.35,
    radial_grid=False,  # Default to False for cleaner diagrams
    radial_grid_alpha=0.1,
    radial_grid_color='gray',
    radial_grid_style='--',
    radial_grid_width=0.5,
    ...
    # ============================================
    # COMPLETE GRID CONTROL - FIX APPLIED
    # ============================================
    
    # First, completely disable matplotlib's default grid
    ax.grid(False)
    
    # Only show radial grid circles if explicitly requested
    if radial_grid:
        # Add custom radial grid circles with full control
        radial_levels = np.linspace(0, radial_base + 1.5, radial_tick_count)
        for r in radial_levels[1:]:  # Skip the center (0)
            circle = Circle(
                (0, 0), r, transform=ax.transData._b,
                fill=False, alpha=radial_grid_alpha,
                edgecolor=radial_grid_color,
                linestyle=radial_grid_style,
                linewidth=radial_grid_width,
                zorder=0.5
            )
            ax.add_patch(circle)
    
    # Configure radial ticks (just the labels, not grid lines)
    if radial_ticks:
...
# ============================================================
# ENHANCED RADAR/SPIDER CHART WITH NUMERICAL LABELS - FIX APPLIED
# ============================================================
...
def create_enhanced_radar_chart(
    ...
    circumferential_label_format='.0f',  # Changed from '.0f°' - degree symbol handled separately
    ...
    # ============================================
    # CONFIGURE NUMERICAL LABELS - FIX APPLIED
    # ============================================
    ...
    # Configure circumferential ticks and labels - FIX APPLIED HERE
    if circumferential_labels:
        ...
        # FIX: Handle format strings that might include degree symbol suffix
        if circumferential_label_format.endswith('°'):
            numeric_format = circumferential_label_format.rstrip('°').strip()
            tick_labels = [f"{angle:{numeric_format}}°" for angle in angle_degrees]
        else:
            tick_labels = [f"{angle:{circumferential_label_format}}°" for angle in angle_degrees]
...
# ============================================================
# MAIN STREAMLIT APPLICATION - ALL FIXES APPLIED
# ============================================================
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("📐 Layout & Spacing", expanded=False):
            ...
            # NEW: Radial grid control for chord diagrams
            if viz_type == "Enhanced Chord Diagram":
                radial_grid = st.checkbox("Show Radial Grid", value=False, 
                                        help="Show concentric circles as grid background (default: off for cleaner diagram)")
                
                if radial_grid:
                    col1, col2 = st.columns(2)
                    with col1:
                        radial_grid_alpha = st.slider("Grid Alpha", 0.0, 1.0, 0.1, 0.05)
                        radial_grid_color = st.color_picker("Grid Color", "#808080")
                    with col2:
                        radial_grid_style = st.selectbox("Grid Style", ["-", "--", ":", "-."], index=1)
                        radial_grid_width = st.slider("Grid Width", 0.1, 2.0, 0.5, 0.1)
            else:
                # For other visualizations, use default value
                radial_grid = False
        ...
        # Special controls for Radar/Radial plots - FIX APPLIED HERE
            ...
            with st.expander("🎯 Radar/Radial Controls", expanded=True):
                ...
                with col2:
                    ...
                    if circumferential_labels:
                        ...
                        # FIX: Changed from [".0f°", ".1f°", ".2f°"] to [".0f", ".1f", ".2f"]
                        circumferential_label_format = st.selectbox(
                            "Circumferential Label Format", 
                            [".0f", ".1f", ".2f"], 
                            index=0,
                            help="Numeric format for angle labels (degree symbol added automatically)"
                        )
        ...
        with st.expander("🎨 Colors & Effects", expanded=False):
            ...
            # FIX: Initialize frame_width before the conditional block
            frame_width = 1.0  # Default value to prevent undefined variable error
            ...
            try:
                if viz_type == "Enhanced Chord Diagram":
                    fig = create_enhanced_chord_diagram(
                        ...
                        # Radial layout - INCLUDING GRID FIX
                        radial_base=radial_base,
                        radial_spacing=radial_spacing,
                        radial_label_offset=label_offset_radius,
                        radial_grid=radial_grid,
                        radial_grid_alpha=radial_grid_alpha if radial_grid else 0.1,
                        radial_grid_color=radial_grid_color,
                        radial_grid_style=radial_grid_style,
                    ...
                    fig = create_enhanced_radar_chart(
                        ...
                        # Numerical label controls - FIX APPLIED
    ...
    with st.expander("📖 Visualization Guide", expanded=False):
        ...
        ### **Enhanced Chord Diagram - Grid Fix**
        
        **The Problem:**
        - Previously, chord diagrams showed unwanted concentric circles (radial grid)
        - These were matplotlib's default polar grid that wasn't properly disabled
        
        **The Solution:**
        - Added complete grid control with `Show Radial Grid` checkbox
        - Default setting is **OFF** for cleaner chord diagrams
        - When enabled, you can customize:
          - Grid transparency (alpha)
          - Grid color
          - Grid line style (solid, dashed, dotted, dash-dot)
          - Grid line width
        ...
        **Publication-Ready Chord Diagram:**
        1. Set `Show Radial Grid` = **False**
        2. Set `Show Frame` = **False**
        3. Set `Show Circumferential Labels` = **False**
        4. Enable `Label Halo Effect` for better readability
        5. Export as **PDF** or **SVG** for vector quality
        
        **Exploratory Analysis:**
        1. Enable `Show Radial Grid` for reference
        2. Set `Edge Threshold` to 0.1 to see all connections
        3. Use `coolwarm` colormap for clear positive/negative distinction
        4. Enable `Show Feature Values` in labels
        
        **Presentation Slides:**
        1. Increase figure size to 18x18
        2. Use vibrant colormap like `viridis` or `plasma`
        3. Enable `Node Glow Effect` and `Edge Glow Effect`
        4. Export as **PNG** at 300 DPI
        """)
