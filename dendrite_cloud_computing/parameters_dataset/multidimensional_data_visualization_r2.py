...
def create_correlation_chord_diagram(df_selected, colormap_name='RdBu', 
    ...
    for (i, j), color, width, value in zip(connections, edge_colors, edge_widths, edge_values):
        ...
        verts = [
            ...
            (theta2, radius2),
            (theta2, radius2 + width/10),  # Add slight thickness
            (control_theta, control_radius + width/10),
            (theta1, radius1 + width/10),
            (0, 0),  # Close path
        ]
        codes = [
            Path.MOVETO,
        ...
        for i in range(n_features):
            for j in range(i + 1, n_features):
                ...
                verts = [
                    ...
                    (control_theta, control_radius),
                    (theta2, radius2),
                    (theta2, radius2 + edge_width/15),
                    (control_theta, control_radius + edge_width/15),
                    (theta1, radius1 + edge_width/15),
                    (0, 0),
                ]
                codes = [
                    Path.MOVETO,
...
def main():
    ...
    with st.sidebar:
        ...
        if viz_type in ["Comparative Analysis"]:
            # For comparative analysis, allow selection of multiple row ranges
        ...
        try:
            if viz_type == "Correlation Chord Diagram":
                ...
                if len(selected_rows) > 20:
                    ...
                    groups = np.array_split(selected_rows, 3)
                else:
                ...
                for idx, (col, fig) in enumerate(zip(cols, figs)):
                    with col:
