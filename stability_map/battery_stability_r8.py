...
threshold_label_pad = st.sidebar.slider("Threshold Label Horizontal Pad", 0.01, 0.30, 0.12, step=0.01)
...
# FIXED: Use plain text for scaling prefix, math mode ONLY for the symbol itself
# This avoids mathtext parser errors with Unicode characters like ×
default_names = [
    r'$A_s$' + chr(10) + '(x 10$^{2}$ J mol$^{-1}$)',
    r'$\kappa$' + chr(10) + '(x 10$^{-10}$ J m$^{-1}$)',
    r'$U$' + chr(10) + '(V)',
    r'$\psi$' + chr(10) + '(x 10$^{-3}$ s$^{-1}$)'
]
