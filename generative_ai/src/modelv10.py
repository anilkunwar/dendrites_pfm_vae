...
class VAE(nn.Module):
    """
    forward 输出：
      recon, mu_q, logvar_q, ctr_pred, z
    """
    def __init__(
        self,
...
# ==========================================================
# Discriminator (新增：用于对抗损失)
# - 输出 logits（不做 sigmoid），训练时用 BCEWithLogitsLoss
# ==========================================================
class Discriminator(nn.Module):
    """
    轻量 PatchGAN 风格鉴别器：输入 [B,C,H,W] -> 输出 [B,1]
    """
    def __init__(self, in_channels=3, base_channels=64, img_size=(64, 64)):
        super().__init__()
        H, W = img_size

        def block(cin, cout, k=4, s=2, p=1, bn=True):
        ...
        self.features = nn.Sequential(
            block(in_channels, base_channels, bn=False),           # H/2
            block(base_channels, base_channels * 2),               # H/4
            block(base_channels * 2, base_channels * 4),           # H/8
            block(base_channels * 4, base_channels * 8),           # H/16
        )

        # 根据输入尺寸推断 flatten dim
        with torch.no_grad():
        ...
        self.head = nn.Sequential(
            nn.Flatten(),
            nn.Linear(flat_dim, base_channels * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(base_channels * 4, 1)  # logits
        )
