...
def soft_dice_loss(pred, target, eps=1e-6):
    """
    pred, target: [B, H, W], values in [0,1]
    """
...
class VAELoss(nn.Module):
    """
    L2 reconstruction + beta * KL + Soft Dice(channel 0, smoothed)
    """
    ...
    def forward(self, recon_x, x, kl, freeze=False):
        ...
        # --------------------------------------------------
        # 3. Soft Dice loss (only channel 0)
        # --------------------------------------------------
        pred_c0 = recon_x[:, 0, :, :]
        gt_c0 = x[:, 0, :, :]

        dice_loss = soft_dice_loss(pred_c0, gt_c0)

        # --------------------------------------------------
        # total
        # --------------------------------------------------
        total_loss = recon_loss + kl_loss + self.dice_weight * dice_loss
