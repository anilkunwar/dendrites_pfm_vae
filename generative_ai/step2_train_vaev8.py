...
def kl_standard_normal_mixture(alpha, mu_q, logvar_q):
    kl_each = 0.5 * torch.sum(
        torch.exp(logvar_q) + mu_q ** 2 - 1.0 - logvar_q,
        dim=-1
    )
    return torch.sum(alpha * kl_each, dim=-1)
...
def main(args):
    ...
    exp_name = (
        f"V8_"
        f"lat={args.latent_size}_K={args.n_components}_"
    ...
    for epoch in range(args.epochs):
        ...
        for x, y, _, xo in train_loader:
            ...
            recon, alpha, mu_q, logvar_q, ctr_pred, _ = model(x)

            recon_loss = multiscale_recon_loss(recon, xo)
            kl_loss = kl_standard_normal_mixture(alpha, mu_q, logvar_q).mean()
