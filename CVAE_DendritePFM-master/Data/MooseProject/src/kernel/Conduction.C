...
registerMooseObject("newtApp", Conduction);
...
: DerivativeMaterialInterface<Kernel>(parameters),
	_cp_var(coupled("cp")),
	_cp(coupledValue("cp")),
  _cv_var(coupled("cv")),   // cv在残差计算中没有用到，但是在反对角雅可比计算中用到了。因为要考虑Q和QM依赖于各个可能的Variable的情况。
  _cv(coupledValue("cv")),
...
Real
Conduction::computeQpJacobian() // 对操作变量的求导（这里假设了Q和QM不依赖于u）
{
  return _Q[_qp]*_grad_phi[_j][_qp]* _grad_test[_i][_qp];
}
Real
Conduction::computeQpOffDiagJacobian(unsigned int jvar) // 对其余耦合变量的求导
{
   if (jvar == _cp_var)
